...
public class BookingController {
    ...
    /** Manual checkout by manager/admin. */
    @PutMapping("/manager/{id}/checkout")
    @PreAuthorize("hasAnyRole('LOT_MANAGER', 'ADMIN')")
    public ResponseEntity<BookingResponseDTO> manualCheckOut(@PathVariable Long id, Authentication auth) {
        String performer = (String) auth.getPrincipal();
        log.info("PUT /api/bookings/manager/{}/checkout performer={}", id, performer);
        return ResponseEntity.ok(service.manualCheckOut(id, performer));
    }

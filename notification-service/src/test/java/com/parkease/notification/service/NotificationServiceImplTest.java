...
class NotificationServiceImplTest {
    ...
    void shouldSendInAppNotificationSuccessfully() {
        ...
        request.setRecipientEmail("sanvi@test.com");
    ...
    void shouldGetMyNotificationsSuccessfully() {
        when(repo.findByRecipientEmailOrderBySentAtDesc("sanvi@test.com"))
                .thenReturn(List.of(notification));
        when(mapper.toDTO(notification)).thenReturn(responseDTO);

        List<NotificationResponseDTO> result =
                service.getMyNotifications("sanvi@test.com");
    ...
    void shouldGetUnreadCountSuccessfully() {
        when(repo.countByRecipientEmailAndIsReadFalse("sanvi@test.com"))
                .thenReturn(5);

        int result = service.getUnreadCount("sanvi@test.com");
    ...
    void shouldMarkAsReadSuccessfully() {
        ...
        NotificationResponseDTO result =
                service.markAsRead(1L, "sanvi@test.com");
    ...
    void shouldMarkAllAsReadSuccessfully() {
        when(repo.markAllAsRead("sanvi@test.com")).thenReturn(4);

        int result = service.markAllAsRead("sanvi@test.com");
    ...
    void shouldDeleteNotificationSuccessfully() {
        ...
        service.deleteNotification(1L, "sanvi@test.com");
    ...
    @Test
    void shouldThrowWhenDeletingOthersNotification() {
        when(repo.findById(1L)).thenReturn(Optional.of(notification));
    ...
    @Test
    void shouldReturnDtoWhenAlreadyRead() {
        notification.setRead(true); // already read
        when(repo.findById(1L)).thenReturn(Optional.of(notification));
        when(mapper.toDTO(notification)).thenReturn(responseDTO);

        // Should not call repo.save again — just return existing state
        NotificationResponseDTO result = service.markAsRead(1L, "sanvi@test.com");
    ...
    void shouldThrowWhenNotificationNotFound() {
        ...
        assertThrows(ResourceNotFoundException.class,
                () -> service.markAsRead(99L, "sanvi@test.com"));
    }

    @Test
    void shouldReturnUnreadNotifications() {
        when(repo.findByRecipientEmailAndIsReadFalseOrderBySentAtDesc("sanvi@test.com"))
                .thenReturn(List.of(notification));
        when(mapper.toDTO(notification)).thenReturn(responseDTO);

        List<NotificationResponseDTO> result = service.getUnread("sanvi@test.com");

        assertEquals(1, result.size());
    }

    @Test
    void shouldThrowWhenDeleteNotificationNotFound() {
        when(repo.findById(99L)).thenReturn(Optional.empty());

        assertThrows(ResourceNotFoundException.class,
                () -> service.deleteNotification(99L, "sanvi@test.com"));
    }
}
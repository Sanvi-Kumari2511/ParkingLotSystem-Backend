...
public class ParkingLotController {
     ...
     * Get all approved lots — for internal analytics use (service-to-service call).
     */
    @GetMapping("/admin/all")
    public ResponseEntity<List<ParkingLotResponseDTO>> getAllApprovedLots() {
        log.info("GET /api/lots/admin/all");
        return ResponseEntity.ok(service.getAllApprovedLots());
    }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ParkingLotResponseDTO> approveLot(
            @PathVariable Long id,
            @RequestParam(required = false) String feedback,
            Authentication auth) {
        String adminEmail = (String) auth.getPrincipal();
        log.info("PUT /api/lots/admin/{}/approve by {}", id, adminEmail);
        return ResponseEntity.ok(service.approveLot(id, feedback, adminEmail));
    }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ParkingLotResponseDTO> rejectLot(
            @PathVariable Long id,
            @RequestParam(required = false) String feedback,
            Authentication auth) {
        String adminEmail = (String) auth.getPrincipal();
        log.info("PUT /api/lots/admin/{}/reject by {}", id, adminEmail);
        return ResponseEntity.ok(service.rejectLot(id, feedback, adminEmail));
    }

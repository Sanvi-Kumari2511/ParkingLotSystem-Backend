...
public class AnalyticsController {
    ...
    /**
     * GET /api/analytics/platform/revenue?from=2026-04-01&to=2026-04-13
     * Platform-wide revenue report for a custom date range.
     */
    @GetMapping("/platform/revenue")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<RevenueReportDTO> getPlatformRevenue(
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate from,
        ...
        log.info("GET /api/analytics/platform/revenue from={} to={}", start, end);
        String token = request.getHeader("Authorization");
        return ResponseEntity.ok(service.getPlatformRevenueReport(start, end, token));
    }
    ...
    /**
     * POST /api/analytics/internal/audit
     * Logs a critical action. Called by other microservices.
     */
    @PostMapping("/internal/audit")
    public ResponseEntity<Void> logAuditAction(
            @RequestBody com.parkease.analytics.dto.request.AuditLogRequestDTO requestDTO,
            HttpServletRequest request) {
        String token = request.getHeader("Authorization");
        service.logAction(requestDTO, token);
        return ResponseEntity.ok().build();
    }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<List<com.parkease.analytics.dto.response.AuditLogResponseDTO>> getAuditLogs() {
        log.info("GET /api/analytics/admin/audit");
